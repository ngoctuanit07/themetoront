<?php
class Sns_Toronto_Block_Mainmenu extends Mage_Catalog_Block_Navigation {
    ...
    protected function _getMenuItemHtml($category, $level = 0, $isLast = false, $isFirst = false) {
        ...
        if($level==0){
            $catdetail = Mage::getModel('catalog/category')->load($category->getId());
            if($catdetail->getData('toronto_groupsubcat')) {
                $this->_isgroup = true;
                $this->_groupitemwidth = $catdetail->getData('toronto_subcat_colw');
            } else {
                $this->_isgroup = false;
            }
        }
        ...
        if($level==0){
            $menutypes = $catdetail->getData('toronto_menutype');
            $rightblock = $this->_getCatBlock($catdetail, 'toronto_block_r');
            $topblock = $this->_getCatBlock($catdetail, 'toronto_block_t');
            $bottomblock = $this->_getCatBlock($catdetail, 'toronto_block_b');
            $hidelink = $this->_getCatBlock($catdetail, 'toronto_menulink');
        ...
        if ($level == 0) {
        	$submenuwidth = $catdetail->getData('toronto_subcat_w');
    ...
    public function getMenuHtml($momenu = false, $level = 0) {
        ...
        $customitems	=	Mage::helper('toronto/data')->getField('menu_customItems');
